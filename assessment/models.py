...
class AssessmentReferee(models.Model):
    match = models.ForeignKey(
        Match, on_delete=models.CASCADE, verbose_name=_('match'))
    referee = models.ForeignKey(
        Referee, on_delete=models.CASCADE, verbose_name=_('referee'))
    user = models.ForeignKey(
        CustomUser, on_delete=models.CASCADE, verbose_name=_('user'), null=True)
    team = models.ForeignKey(
        Team, on_delete=models.CASCADE, verbose_name=_('team'))
    questionnaire = models.ManyToManyField(
        QuestionR, blank=True, verbose_name=_('questionnaire'))
    confirm = models.BooleanField(_('confirmation'), default=False)
    datetime_confirm = models.DateTimeField(
        _('datetime of confirmation'), blank=True, null=True)
...
class AssessmentMatch(models.Model):
    ...
    match = models.ForeignKey(
        Match, on_delete=models.CASCADE, verbose_name=_('match'))
    assessment_referees = models.ManyToManyField(
        AssessmentReferee, blank=True, verbose_name=_('Assessment referees'))
    ...
    def create_assessments_referee(self):
        for r in self.match.get_referees():
            ar = AssessmentReferee(
                match=self.match, referee=r, team=self.team, refer_am=self)
            ar.save()
