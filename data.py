...
print("Create teams")
teams = ["RDM Mouscron", "CNT Tournai", "AZC Antwerpen",
         "RSCM Mechelen", "ENLWP La louvière", "MZV Eeklo", "RBP Poséidon"]
for team in teams:
    t = Team(name=team)
    t.save()
    comp.teams.add(t)
...
dates = ['6/10/2018', '13/10/2018', '20/10/2018', '27/10/2018', '3/11/2018', '10/11/2018', '17/11/2018', '24/11/2018',
         '1/12/2018', '8/12/2018', '15/12/2018', '19/01/2019', '26/01/2019', '2/02/2019', '2/03/2019', '9/03/2019']
for matches in list(round_robin(teams, sets=len(teams) * 2 - 2)):
    if type(matches) is list:
        ...
        for home, away in matches:
            ...
            dt_match = pytz.timezone(settings.TIME_ZONE).localize(
                datetime.strptime("%s 20:00" % dates[0], "%d/%m/%Y %H:%M"))
            m1 = Match(competition=comp, teamH=teamH,
                       teamA=teamA, datetime=dt_match)
            m1.save()
            m1.teams.add(teamH, teamA)
            m1.referees.add(Referee.objects.get(
                id=int(ref_1[0])), Referee.objects.get(id=int(ref_1[1])))
            del ref_1[0]
            del ref_1[0]
            m1.save()
            ...
            dt_match = pytz.timezone(settings.TIME_ZONE).localize(
                datetime.strptime("%s 20:00" % dates[len(teams)], "%d/%m/%Y %H:%M"))
            m2 = Match(competition=comp, teamH=teamA,
                       teamA=teamH, datetime=dt_match)
            m2.save()
            m2.teams.add(teamH, teamA)
            m2.referees.add(Referee.objects.get(
                id=int(ref_2[0])), Referee.objects.get(id=int(ref_2[1])))
            del ref_2[0]
            del ref_2[0]
            m2.save()
            m1.save()
...
for i in random.sample(range(0, len(a)), 20+random.randint(0, len(a)-20)):
    ...
    for qr in a[i].get_sorted_questions():
        qr.answer = random.randint(
            qr.question.min_value, qr.question.max_value)
        qr.save()
    a[i].save()
